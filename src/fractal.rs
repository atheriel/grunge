...
/// PinkNoise is generated by calculating the contribution of a number of individual
/// `octaves` of noise samples, and then adding them together. PinkNoise is a kind of
/// fractal noise, because the contributions are self-similar
///
/// Its output takes the form
///
/// $ M(\bold{x}) = p N(f \bold{x}) + p^2 N(fl \bold{x}) + \ldots + p^n N(fl^{n - 1} \bold{x}) $
///
/// where $p, f, l$ are persistence, frequency, and lacunarity, respectively, `noise`
/// is the noise function, and $\bold{x}$ is the vector of input coordinates.
...
impl Default for PinkNoise {
    fn default() -> PinkNoise {
        PinkNoise { seed: 0, frequency: 1.0, persistence: 0.5, lacunarity: 2.0, octaves: 6 }
    }
}
